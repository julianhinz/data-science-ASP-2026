...
- `code/01-spatial_heterogeneity.R` — Merges Colombian customs import data (2018) with department-level presidential election results. Produces five figures and a summary table.
...
- Create output directories at script start
- Save figures as both PNG and PDF
- Clean up large objects with `rm()` after use

## Testing and validation

- R scripts should run without errors via `Rscript code/<script>.R` from the module root
- Check that output files are created in `output/` as expected
